...
export interface IStorage {
  ...
  recycleEquipment(
    userId: string,
  ): Promise<{ count: number; stonesGained: number }>;
...
export class DatabaseStorage implements IStorage {
  ...
  async getEquipment(
    userId: string,
  ): Promise<(Equipment & { cards: Card[] })[]> {
    const items = await db
      .select()
      .from(equipment)
      .where(eq(equipment.userId, userId));
    const equipmentWithCards = await Promise.all(
      items.map(async (item) => {
        try {
          const itemCards = await db
            .select()
            .from(cards)
            .where(eq(cards.equipmentId, item.id));
  ...
  async insertCardIntoEquipment(
    ...
    const existingCards = await db
      .select()
      .from(cards)
      .where(eq(cards.equipmentId, equipmentId));
  ...
  async getCampaignEvents(userId: string): Promise<CampaignEvent[]> {
    return await db
      .select()
      .from(campaignEvents)
      .where(eq(campaignEvents.userId, userId));
  }
    ...
    if (quest) {
      if (!quest.isClaimed) {
        await db
          .update(userQuests)
          .set({
            progress: (quest.progress || 0) + increment,
            lastUpdated: new Date(),
          })
  ...
  async claimQuest(
    ...
    const QUEST_DEFS_LOOKUP: Record<
      string,
      { goal: number; rewardType: string; amount: number }
    > = {
